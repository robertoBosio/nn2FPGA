...
class TestYoloHead(BaseHLSTest):
    ...
    def generate_config_file(self, config_dict):
        ...
        qlinear_reshape = helper.make_node(
            "QuantizeLinear",
            inputs=["X_reshaped", "X_scale", "X_zp"],
            outputs=["X_q_reshaped"],
        )

        dqlinear_reshape = helper.make_node(
            "DequantizeLinear",
            inputs=["X_q_reshaped", "X_scale", "X_zp"],
            outputs=["X_q_reshaped_dq"],
        )

        transpose = helper.make_node(
            "Transpose",
            inputs=["X_q_reshaped_dq"],
            outputs=["X_transposed"],
            perm=[0, 2, 1, 3],  # (N,C,H,W) -> (N,H,C,W)
        )

        qlinear_transpose = helper.make_node(
            "QuantizeLinear",
            inputs=["X_transposed", "X_scale", "X_zp"],
            outputs=["X_q_transposed"],
        )

        dqlinear_transpose = helper.make_node(
            "DequantizeLinear",
            inputs=["X_q_transposed", "X_scale", "X_zp"],
            outputs=["X_q_transposed_dq"],
        )
    ...
    def test_yolov5nu(self, hls_steps):
        np.random.seed(0)
        config_dict = {
            "INPUT_DATAWIDTH": 8,
            "OUTPUT_DATAWIDTH": 8,
