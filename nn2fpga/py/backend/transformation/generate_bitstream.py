...
def dump_tcl_script(top_name, part_name, frequency, hls_version, only_synthesize=False) -> str:
    ...
    if float(hls_version) > 2025:
        lines.append(
            'open_component -reset "hlsproj" -flow_target vivado',
        )
    else:
        lines.extend(
            [
                'open_project -reset "hlsproj"',
    ...
    lines.extend(
        [
            ...
            'csynth_design',
            'export_design -format ip_catalog -ipname "{top_name}" -library "ml" -vendor "polito.nn2FPGA" -version "1.0" -description "Generated by nn2FPGA"' if not only_synthesize else '', 
            'exit',
        ]
    )
...
class GenerateBitstream(Transformation):
    ...
    def apply(self, model: ModelWrapper) -> tuple[ModelWrapper, bool]:
        ...
        tcl_script = dump_tcl_script(
            ...
            only_synthesize=self.only_synthesize,
        )
