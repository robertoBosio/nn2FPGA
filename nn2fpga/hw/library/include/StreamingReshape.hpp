...
public:
  ...
  void run(hls::stream<TInputWord> i_data[W_PAR],
    ...
    STREAMINGRESHAPE_RUN_LOOP:
      for (size_t i_ch = 0; i_ch < IN_CH / CH_PAR; i_ch++) {
#pragma HLS pipeline II = 1
        StreamingReshape::pipeline_body(i_data, o_data);
      }
    }
  }
  ...
  ActorStatus step(hls::stream<TInputWord> i_data[W_PAR],
    ...
    if (firing_condition) {
      ...
      StreamingReshape::pipeline_body(i_data, instant_output_stream);
