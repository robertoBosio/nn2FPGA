...
public:
  ...
  void step_init(size_t pipeline_depth = 1) {
    (void)pipeline_depth; // Suppress unused variable warning
  }
  ...
  void run(hls::stream<TOutputWord> &output_data_stream) {
    // Run the DMA until it has produced the required number of tokens.
    for (size_t i = 0; i < STEP_words_per_tensor; ++i) {
      output_data_stream.write(TOutputWord());
    }
  }
...
public:
  ...
  void run(hls::stream<TInputWord> &input_data_stream) {
    // Run the DMA until it has consumed the required number of tokens.
    for (size_t i = 0; i < STEP_words_per_tensor; ++i) {
      TInputWord input_data = input_data_stream.read();
      (void)input_data; // Suppress unused variable warning
    }
