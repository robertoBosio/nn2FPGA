...
void wrap_run(
    hls::stream<test_config::TWord> input_data_stream[test_config::W_PAR],
    hls::stream<test_config::TWord> output_data_stream0[test_config::W_PAR],
    hls::stream<test_config::TWord> output_data_stream1[test_config::W_PAR]) {
  TensorDuplicator<test_config::TWord, test_config::IN_HEIGHT,
...
bool test_run(){
  ...
  hls::stream<test_config::TWord> out_stream0[test_config::W_PAR];
  hls::stream<test_config::TWord> out_stream1[test_config::W_PAR];
  ...
  for (size_t i = 0; i < test_config::IN_HEIGHT * test_config::IN_WIDTH;
    ...
    for (size_t ch = 0; ch < test_config::IN_CH; ch += test_config::CH_PAR) {
      for (size_t w_par = 0; w_par < test_config::W_PAR; w_par++) {
        test_config::TWord output_word0 = out_stream0[w_par].read();
        test_config::TWord output_word1 = out_stream1[w_par].read();
        for (size_t ch_par = 0; ch_par < test_config::CH_PAR; ch_par++) {
...
bool test_step() {
  ...
  TensorDuplicator<
      ...
      tensor_duplicator; 
  tensor_duplicator.step_init(test_config::PIPELINE_DEPTH);
  ...
  while (true) {
    ...
    ActorStatus actor_status = tensor_duplicator.step(in_stream, out_stream0, out_stream1);
