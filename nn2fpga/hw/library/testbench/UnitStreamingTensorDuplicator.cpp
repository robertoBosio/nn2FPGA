...
void wrap_run(
    hls::stream<test_config::TWord> input_data_stream[test_config::DIM1_UNROLL],
    hls::stream<test_config::TWord>
        output_data_stream0[test_config::DIM1_UNROLL],
    hls::stream<test_config::TWord>
        output_data_stream1[test_config::DIM1_UNROLL]) {
  StreamingTensorDuplicator<test_config::TWord, test_config::DIM0, test_config::DIM1,
                   test_config::DIM2, test_config::DIM1_UNROLL,
                   test_config::DIM2_UNROLL>
      tensor_duplicator;
  tensor_duplicator.run<0>(input_data_stream, output_data_stream0,
                           output_data_stream1);
}
...
bool test_run() {
  ...
  hls::stream<test_config::TWord> out_stream0[test_config::DIM1_UNROLL];
  hls::stream<test_config::TWord> out_stream1[test_config::DIM1_UNROLL];
  ...
  for (size_t i_dim01 = 0; i_dim01 < test_config::DIM0 * test_config::DIM1;
    ...
    for (size_t i_dim2 = 0; i_dim2 < test_config::DIM2;
      ...
      for (size_t i_dim1_par = 0; i_dim1_par < test_config::DIM1_UNROLL;
           i_dim1_par++) {
        test_config::TWord output_word0 = out_stream0[i_dim1_par].read();
        test_config::TWord output_word1 = out_stream1[i_dim1_par].read();
        for (size_t i_dim2_par = 0; i_dim2_par < test_config::DIM2_UNROLL;
             i_dim2_par++) {
          bool cmp0 = (output_word0[i_dim2_par] ==
                       (i_dim01 + i_dim1_par) * test_config::DIM2 + i_dim2 +
                           i_dim2_par);
          bool cmp1 = (output_word1[i_dim2_par] ==
                       (i_dim01 + i_dim1_par) * test_config::DIM2 + i_dim2 +
                           i_dim2_par);
          if (!cmp0 || !cmp1) {
            std::cout << "Mismatch at index (i_dim01=" << i_dim01
...
bool test_step() {
  ...
  StreamingTensorDuplicator<test_config::TWord, test_config::DIM0, test_config::DIM1,
                            test_config::DIM2, test_config::DIM1_UNROLL,
                            test_config::DIM2_UNROLL>
      streaming_tensor_duplicator;
  streaming_tensor_duplicator.step_init(test_config::PIPELINE_DEPTH);
  ...
  while (true) {
    ...
    for (size_t i_dim1_par = 0; i_dim1_par < test_config::DIM1_UNROLL;
         i_dim1_par++) {
      test_config::TWord input_struct;
      in_stream[i_dim1_par].write(input_struct);
    }

    ActorStatus actor_status =
        streaming_tensor_duplicator.step(in_stream, out_stream0, out_stream1);
