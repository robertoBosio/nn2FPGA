...
void wrap_run(hls::stream<TInputWord> i_data[test_config::LINE_PAR],
  ...
  StreamingYoloHead<
      TInputWord, test_config::TInput, test_config::TProb, TOutputWord, test_config::TOutput,
      test_config::TLUT, test_config::TDen, test_config::TDiv, test_config::TAcc,
      test_config::ProbQuantizer, test_config::OutQuantizer, test_config::LUT_SIZE,
      test_config::OUT_HEIGHT * test_config::OUT_WIDTH, test_config::OUT_CH,
      test_config::LINE_PAR, test_config::REDUCE_PAR>
      streaming_yolo_head;
  streaming_yolo_head.run<0>(i_data, LUTmem, o_data);
}
...
bool test_run() {
  ...
  for (size_t i_w = 0; i_w < test_config::OUT_WIDTH;
    ...
    for (size_t i_h = 0; i_h < test_config::OUT_HEIGHT; i_h++) {
      for (size_t i_line_par = 0; i_line_par < test_config::LINE_PAR;
        ...
        bool cmp = output_word[0] -
                test_config::output_tensor[0][0][i_h][i_w + i_line_par] < 2;
        if (!cmp) {
          // std::cout << "Mismatch at index (i_h=" << i_h
          //           << ", i_w=" << i_w + i_line_par << ", ch=" << 0
          //           << "): " << output_word[0] << " != "
          //           << test_config::output_tensor[0][0][i_h][i_w + i_line_par]
          //           << std::endl;
        }
        flag &= cmp;
      }
  ...
  std::cout << out_stream[0].size() << " output elements read." << std::endl;
...
bool test_step() {
  ...
  StreamingYoloHead<
      TInputWord, test_config::TInput, test_config::TProb, TOutputWord, test_config::TOutput,
      test_config::TLUT, test_config::TDen, test_config::TDiv,
      test_config::TAcc, test_config::ProbQuantizer, test_config::OutQuantizer, test_config::LUT_SIZE,
      test_config::OUT_HEIGHT * test_config::OUT_WIDTH, test_config::OUT_CH,
      test_config::LINE_PAR, test_config::REDUCE_PAR>
      streaming_yolo_head;
  streaming_yolo_head.step_init(test_config::PIPELINE_DEPTH);
  ...
  while (true) {
    ...
    ActorStatus actor_status =
        streaming_yolo_head.step(in_stream, test_config::LUTmem, out_stream);
