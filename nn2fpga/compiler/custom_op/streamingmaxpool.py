...
class StreamingMaxPool(NN2FPGAOp, DSECapable):
    ...
    def __get_quantizer(self, input_type, output_type) -> str:
        """ Returns the quantizer type for the StreamingConv operation. """

        if (
            self.__is_power_of_two(input_type.scale)
            and self.__is_power_of_two(output_type.scale)
        ):
            shift = -1 * (
                int(np.log2(input_type.scale))
                - int(np.log2(output_type.scale))
            )
            return f"DequantQuantPo2<{shift}, {input_type.get_hls_data_type()}, {output_type.get_hls_data_type()}>"
        else:
            raise ValueError(
    ...
    def __get_object_declaration(self, model) -> cpp_object:
        ...
        StreamingMaxPool = cpp_object(
            ...
            template_args=[
                ...
                (point.dim2_unroll, "CH_PAR"),
                (point.dim1_unroll, "W_PAR"),
            ],
        )
    ...
    def accepted_input_layout(self) -> tuple | None:
        """ StreamingMaxPool accepts only NHWC layout. """
        return (0, 2, 3, 1)

    def produced_output_layout(self, input_layout: tuple | None) -> tuple | None:
        """ StreamingMaxPool produces only NHWC layout. """
        return (0, 2, 3, 1)
    ...
    def get_dsps(self, model: ModelWrapper) -> int:
        ...
        point = self.__current_dse_point()
        return (point.dim2_unroll * point.dim1_unroll)
    ...
    def get_dse_points(self, model: ModelWrapper) -> list["StreamingMaxPool.DSEPoint"]:
        ...
        for dim2_unroll in self.divisors([output_shape[-1]], output_shape[-1]):
            for dim1_unroll in self.divisors([output_shape[-2]], output_shape[-2]):
                ...
                DSE_points.append(
                    self.DSEPoint(
                        dim2_unroll, dim1_unroll, kernel_height, kernel_width
                    )
                )
