...
class InfiniteThroughputDMA(NN2FPGAOp):
    ...
    def __get_object_declaration(self) -> str:
        """ Get the internal cpp object declarations for the InfiniteThroughputDMA node.
        Args:
        ...
        InfiniteThroughputDMAObject = cpp_object(
            class_name="InfiniteThroughputDMA",
            obj_name=self.onnx_node.name,
            template_args=[f"ap_axiu<{axi_bitwidth}, 0, 0, 0>"],
            constructor_args=[words_per_tensor],
        )
        return InfiniteThroughputDMAObject.generate_declaration()

    def __generate_step_call(self) -> str:
        """Generate the HLS step call for the InfiniteThroughputDMA node.
        Args:
            model (ModelWrapper): The model with quantization information.
        Returns:
        ...
        step = cpp_function(
            ...
            return_type="void",
            arguments=((f"input_data_stream", f"hls::stream<TInputWord>"),),
        )
    ...
    def accepted_input_layout(self) -> tuple | None:
        """ InfiniteThroughputDMA supports any input layout."""
        return None

    def produced_output_layout(self, input_layout: tuple | None) -> tuple | None:
        """ InfiniteThroughputDMA produces the same layout as its input (transparent)."""
        return input_layout

    def lower_to_hls(self, model: ModelWrapper, hls_tag: int):
        """
        Lower the InfiniteThroughputDMA node to HLS kernels.
        Args:
          model: ModelWrapper
        ...
        fifos = {}
        fifos[self.onnx_node.input[0]] = TensorFifo(
            depth=0,
        ...
        hls_kernel = HLSKernel.make_node(
            ...
            outputs=[],
            name=f"{self.onnx_node.name}_{self.onnx_node.input[0]}_hls",
            domain="nn2fpga.compiler.custom_op",
            original_op_type="InfiniteThroughputDMA",
    ...
    def get_latency(self, model: ModelWrapper) -> int:
        """ Estimate the latency of the InfiniteThroughputDMA.
        Args:
            model (ModelWrapper): The model with quantization information.
        Returns:
            int: Estimated latency in clock cycles.
        """
        return self.get_nodeattr("words_per_tensor")

    def get_brams(self, model: ModelWrapper) -> int:
        """ Estimate the BRAM usage of the InfiniteThroughputDMA.
    ...
    def get_dsps(self, model: ModelWrapper) -> int:
        """ Estimate the DSP usage of the InfiniteThroughputDMA.
    ...
    def has_linebuffer(self) -> bool:
        """ Check if the InfiniteThroughputDMA operation requires a line buffer.
        Returns:
