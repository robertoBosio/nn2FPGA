...
void wrap_run(hls::stream<TInputWordA> i_data0[test_config::W_PAR],
              const test_config::TInputB constant_input,
  ...
  StreamingConstMul<TInputWordA, test_config::TInputA,
               ...
               test_config::CH_PAR>
      mul;
  mul.run<0>(i_data0, constant_input, o_data);
}
...
bool test_run() {
  ...
  for (size_t h = 0; h < test_config::IN_HEIGHT; h++) {
        ...
        for (size_t i_w_par = 0; i_w_par < test_config::W_PAR; i_w_par++) {
          ...
          for (size_t i_ch_par = 0; i_ch_par < test_config::CH_PAR;
            ...
            input_data0[i_ch_par] =
                test_config::input_tensorA[0][i_ich + i_ch_par][h][w + i_w_par];
          }
    ...
    // Run the operator
  wrap_run(i_data0, test_config::constant, o_data);
...
bool test_step() {
  ...
  StreamingConstMul<TInputWordA, test_config::TInputA,
               ...
               test_config::CH_PAR>
      mul;
  mul.step_init(test_config::PIPELINE_DEPTH);
  ...
  while (true) {
    ...
    ActorStatus actor_status =
        mul.step(i_data0, test_config::constant, o_data);
