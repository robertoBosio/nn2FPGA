...
class StreamingTranspose {
  struct StepState {
    ...
    size_t depth = 0;
    PipelineDelayBuffer<TOutputWord> delayed_output[1];
    ...
    void init(size_t depth) {
      ...
      actor_status = ActorStatus(
          1,
          2 * (IN_CH * IN_WIDTH *
               IN_HEIGHT)); // 2 comes from the fact that we have 2 passes
                            // (write and read) with same number of iterations
      delayed_output[0] = PipelineDelayBuffer<TOutputWord>(depth);
      initialized = true;
      this->depth = depth;
    }
  };
...
#pragma HLS PIPELINE II = 1
  ...
  ActorStatus step(hls::stream<TInputWord> in_data[1],
    ...
    if (firing_condition) {
      ...
      hls::stream<TOutputWord> instant_out[1];
      StreamingTranspose::pipeline_body(in_data, instant_out, st.buf, k, w, ch,
                                        pass);
      ...
      if (!instant_out[0].empty()) {
        st.delayed_output[0].push(instant_out[0].read(), true);
      } else {
        st.delayed_output[0].push(TOutputWord(), false);
      }
    }

    st.actor_status.advance();
    TOutputWord out_word;
    if (st.delayed_output[0].pop(out_word)) {
...
private:
  static void pipeline_body(hls::stream<TInputWord> in_data[1],
                            hls::stream<TOutputWord> out_data[1],
                            TInput buf[IN_CH * IN_WIDTH * IN_HEIGHT], int k,
                            int w, int ch, int pass) {
#pragma HLS inline

    // consider matrix[ch][w][k] as the original layout, and matrix[ch][k][w] as
    // the transposed layout
    if (pass == 0) {
      INDEX_T idxRead = ch * IN_WIDTH * IN_HEIGHT + w * IN_HEIGHT + k;
      TInputWord pkt = in_data[0].read();
      buf[idxRead] = pkt[0];
    } else {
